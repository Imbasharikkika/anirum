...
const URL_STUDENTS = "http://localhost:1337/api/students";
const URL_PRESENCES = "http://localhost:1337/api/presences";
const getGroupUrl = ({ id }) =>
  `http://localhost:1337/api/groups/${id}?populate[0]=teacher&populate[1]=days&populate[2]=office&populate[3]=office.address&populate[4]=office.address.district&populate[5]=office.address.district.city&populate[6]=students&populate[7]=students.presences&populate[8]=students.presences.day&populate[9]=students.refuseds.group`;
...
const Group = () => {
  ...
  const groupUrl = getGroupUrl({ id });
  const { loading, error, data, fetchData } = useFetch(groupUrl);
  ...
  const [studentsstar, setStudentsstar] = useState({
    id: undefined,
    level: undefined,
  });
  ...
  .then((res) => {
    let s = 0;
    while (s < res.data.data.length) {
      studentsall.students.push({
        id: res.data.data[s].id,
        label: 
        res.data.data[s].attributes.family[0].toUpperCase() + res.data.data[s].attributes.family.slice(1) + ' ' +
        res.data.data[s].attributes.name[0].toUpperCase() + res.data.data[s].attributes.name.slice(1) + ' ' +
        res.data.data[s].attributes.patronymic[0].toUpperCase() + res.data.data[s].attributes.patronymic.slice(1),
        value: res.data.data[s].attributes.family.toLowerCase() + ' ' +
        res.data.data[s].attributes.name.toLowerCase() + ' ' +
        res.data.data[s].attributes.patronymic.toLowerCase()
        ,
      });
      s++;
    }
  });
  }, [setStudentsall, studentsall]);
  const [ formadd, setFormadd ] = useState(true)

  function submit(e) {
    e.preventDefault();
    Axios.post(URL_STUDENTS, {
      data: {
        groups: id,
        name: student.name,
  ...
  function submit2(e) {
    e.preventDefault();
    let d = 0;
    while (d < studentsstar.length) {
    Axios.put(`${URL_STUDENTS}/${studentsstar[d].id}`, {
      data: {
        groups: id,
        level: student.level,
      },
    ...
    .then((res) => {
      let w = 0;
          while (w < TrialOrLessonPresences.length) {
            Axios.post(URL_PRESENCES, {
              data: {
                student: res.data.data.id,
                day: TrialOrLessonPresences[w],
                price_lesson: pricePresenceLesson[w],
                trial_price: pricePresenceTrial,
                status: "придут",
              },
            });
  ...
  const { months, dayFilterMonth, monthsGroup, actualDayGroupFormatDay } =
    useMemo(
      () => getDates({ data, checkedMonths: checked.months }),
      [data, checked.months]
    );
  ...
  function onChangeStudentall(value) {
    
    let starstudens = []
    let i = 0
    while(i < value.length){
      starstudens.push({id:value[i].id})
      i++
    }
    setStudentsstar(starstudens)
  }
  // актуальные дни
  ...
  const studentsTable = students.map((x) => {
    ...
    return {
      ...
      refuset: x.attributes.refuseds.data.filter(x => x.attributes.group.data.id === data.data.id).length,
      
      // course: studentDates2
      //   .map((x) => x.price_lesson)
      //   .reduce((acc, rec) => (acc += rec), 0),
      course: dayall.map((day) => ({
        course: studentDates2
        .filter(
          (s) =>
      ...
      })).reduce((acc, rec) => (acc += rec.course), 0),
      trial: dayall.map((day) => ({
        trial: studentDates2
        .filter(
          (s) =>
            moment(s.day.data.attributes.title, "YYYY-MM-DD").format(
      ...
      })).reduce((acc, rec) => (acc += rec.trial), 0),
  ...
  const handleChange = (e) => {
    ...
    Axios.put(
      `http://localhost:1337/api/presences/${option.getAttribute("data-id")}`,
      {
        data: {
          status: option.value,
        },
  ...
  const onChangeRefuse = (e) => {
    Axios.post(
      `http://localhost:1337/api/refuseds`,
      {
        data: {
      ...
      <div className="box_goup2">
        ...
        {months.length >= 1 && (
          <div className="form"> {formadd?
            <div>
              <button onClick={()=> setFormadd(!formadd)}>добавить из списка</button>
            ...
            : 
            <div><button onClick={()=> setFormadd(!formadd)}>новый клиент</button>
            <h3 className="form_title">Добавить студента</h3>
            <Select options={studentsall.students} 
            isMulti  onChange={onChangeStudentall}/>
            <form className="form_container" onSubmit={(e) => submit2(e)}>
        ...
        <div className="card">
          ...
          {studentsTable.map((x, index) => (
            ...
            <div className="group_number2" key={x.id}>
                    ...
                    <div>
                      {s.trial ? (
                        <div className="row">
                          ...
                          <select
                            className="form_inpuit3"
                            defaultValue={s.status}
                            onChange={handleChange}
                          >
                            <option value={"придут"} data-id={s.id}>
                              придут
                            </option>
                            <option value={"подошел"} data-id={s.id}>
                              подошел
                            </option>
                            <option value={"пропуск"} data-id={s.id}>
                              пропуск
                            </option> 
                            </select>
                        </div>
              ...
              <div className="title_age_table">
              <div>
              <button 
                    value={x.id}
                    onClick={onChangeRefuse}
                    >Удалить</button>
              </div>
              </div>
            </div>
            ...
            {x.refuset !== 0 &&
            <div >
              {x.dates.filter(s=>s.value === 'записан').length >= 1 &&
            <div className="group_number2" key={x.id}>
              <div className="title_number_table2">{index + 1}</div>
              <div className="title_fio_table">{x.name}</div>
              <div className="title_age_table">{x.age}</div>
                      ...
                      {s.trial ? (
                        <div className="row">
                          ...
                          >
                            ...
                            {/* <option value={'заболели'} data-id={s.id}>заболели</option> */}
                          </select>
                        </div>
                      ) : (
                        <div></div>
              ...
              <div className="title_age_table"><div>
              <div>
                  В отказах
              </div>
              </div>
